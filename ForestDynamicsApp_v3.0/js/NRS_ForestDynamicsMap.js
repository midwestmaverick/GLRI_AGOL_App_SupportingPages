...
function queryDataToInitChart(){
	...
	var data;
	var layers = layerDefs.filter(function(d){if(d.title == $("#lX").text() || d.title == $("#lY").text()){return d}});
...
function queryLyrs(){
	var start = new Date().getTime();
	var data;
	var lyrs = layerDefs.filter(function(d){if(d.title == $("#lX").text() || d.title == $("#lY").text()){return d}});
